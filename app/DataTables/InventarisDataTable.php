...
class InventarisDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        ->addColumn('delete', function($request){    
            return view ('datatable._delete',[
                'model'    => $request,
                'delete_url' => route('inventaris.destroy', $request->id),
                'confirm_message' => 'Yakin ingin menghapus Data Inventaris ' . $request->uraian . '?'
            ]);                 
        })
    ...
    {
        $query = mInventaris::select('m_inventaris.*');
