...
class DisabilitasDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        ...
        ->addColumn('delete', function($request){    
            return view ('datatable._delete',[
                'model'    => $request,
                'delete_url' => route('disabilitas.destroy', $request->id),
                'confirm_message' => 'Yakin ingin menghapus Data Disabilitas ' . $request->nama . '?'
            ]);                 
        })
    ...
    {
        $query = mDisabilitas::select('m_disabilitas.*');
    ...
    protected function getColumns()
    {
        return [
            ...
            (['data'=> 'jenis_dis' ,'name' => 'jenis_kb' , 'title' => 'Jenis Disabilitas']),
    ...
    {
        return 'Disabilitas_' . date('YmdHis');
    }
}
