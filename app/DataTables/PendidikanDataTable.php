...
class PendidikanDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables($query)
            //->addColumn('action', 'pendidikandatatable.action')
            ...
            ->addColumn('delete', function($request){    
                return view ('datatable._delete',[
                    'model'    => $request,
                    'delete_url' => route('pendidikan-admin.destroy', $request->id),
                    'confirm_message' => 'Yakin ingin menghapus Data Pendidikan ' . $request->nama . '?'
                ]);                 
            })
    ...
    {
        $query = mPendidikan::select('m_pendidikans.*');
